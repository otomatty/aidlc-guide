...
| BR-DB-4 | **対応表のメンテは本ツール側**（PRD §12 未決の確定）: 33ステージ+主要用語のエントリを data/ に持ち、aidlc-workflows のバージョン更新時に手動同期。対応表に `sourceVersion` フィールドを持たせ、解決時に返す（消費者が古さを表示可能） | PRD §12 / C-T3 精神 |
...
```json
{
  "sourceVersion": "aidlc 2.6.2 (State Version 8)",
