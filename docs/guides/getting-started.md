...
1. コマンドパレットから **`AIDLC Guide: Setup`** を実行して、セットアップ画面を開きます。
...
完了状態はフォルダごとに記録します。文書参照をスキップしても完了できます。セットアップ画面は起動時には自動表示されません。途中で閉じた場合も、コマンドパレットの **`AIDLC Guide: Setup`** から再開できます。

信頼済みのフォルダでは、登録済みの文書参照連携を起動時に更新します。拡張の更新で変わる MCP の参照先と、拡張が管理する Skill が対象です。実際に更新した場合だけ AI セッションの再起動を案内し、セットアップ画面は開きません。

MCP だけ後から入れたいときは **`AIDLC Guide: Register MCP`** でも同じマージができます。

セットアップ画面の「AI-DLC の診断結果」には、本体の診断を日本語で表示します。「診断を実行」で確認でき、次回からは「診断を再実行」で最新の結果に置き換わります。「正常」「要確認」「問題あり」の件数と、各項目の説明・対処方法を確認してください。設定直後の診断結果もこの欄に表示されます。

日本語表示は本体 2.8.0 / 2.8.1 に対応しています。未対応の文言や形式は省略せず原文を表示し、全出力は「原文を見る」で確認できます。コマンドやファイルパスは原表記のままです。「状態を再確認」は拡張の簡易確認で、本体の診断とは別です。診断には本体の導入とワークスペースの信頼が必要です。
...
設定済みなら、Webview にヘッダー・Now strip・ステージ一覧・成果物マトリクスが出ます。未設定の場合はセットアップ画面が開きます。

### 5. 「はじめに」で画面の見方を知る

セットアップを終えた最初の表示では「はじめに」が出ます。主な画面の見方を実画面で確認でき、「画面を順に案内してもらう」で自分の案件のツアーを始められます。もう一度読むときは右上のメニューの「はじめに」を使います。詳しくは[はじめに・更新情報・ヒント](./onboarding.md)を参照してください。
