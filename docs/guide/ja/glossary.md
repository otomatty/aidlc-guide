...
| **監査証跡 (Audit trail)** | レコードディレクトリの `audit/` にある追記専用ログ。クローンごとのシャード（`<host>-<clone>.md`）を、読む側がグロブして時刻順にマージする。イベント種別は 95。ISO 時刻付きで、意図から本番まで追える。 |
...
| **配布 (Distribution)** | 1 ハーネス向けの生成投影。**コア**を **パッケージャ** が生成する。ソースビルドは無視されるローカル `dist/claude/`、`dist/codex/`、`dist/copilot/`、`dist/cursor/`、`dist/kiro/`、`dist/kiro-ide/`、`dist/opencode/` と、対になるネイティブ `dist-release/<harness>/` を実体化する。リリースはそれらのネイティブ投影を `aidlc-runtime-X.Y.Z.tar.gz` の `runtime/<harness>/` に載せる。メンテナは生成物を手で編集しない。 |
