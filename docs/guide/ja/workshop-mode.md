...
プロジェクトごとの既定スコープは、`.claude/settings.json` に `AWS_AIDLC_DEFAULT_SCOPE=workshop` を設定しても構いません。設定しておけば、各参加者がクローン内で `/aidlc` を実行するだけで Workshop の経路選択を自動的に得られ、フラグを覚える必要がなくなります。詳細は [カスタマイズ § プロジェクトごとの既定スコープ](13-customization.md#per-project-default-scope) を参照してください。
