...
- **継続のリプレイはエンジンが所有します。** 作成済みの `tools/data/harness.json` が `copilot` を指名している現行インストールにおいて、コンテキストが正確で sessionless ではないマーカーに対しては、`continue` はまずネイティブトークンを検証し、後継を構築したうえで、トークンダイジェスト全体をアトミックに比較し、標準出力より先に後継を公開します。同時使用の勝者は 1 つだけです。欠落・不正形式・v1・古い（stale）・`sessionless:` の各マーカーは、既存のステートレスな動作を維持します。Copilot を別のハーネスへ置き換えた場合は、古い Copilot マーカーが残っていても、そのハーネスのステートレスな動作が復元されます。安定した非 Copilot インストールでは、競合したマーカー公開が再検証済みのステートレス継続を拒否することはできません。エンジンは準備済みの指示を返し、代わりに破棄されたベストエフォートのマーカー更新を記録します。インストール済みハーネスのコマンドが実行中の間にそのハーネスを置き換えることは、サポートされたアップグレード経路ではありません。
...
- **Resume と会話待ちはセッション単位です。** Stop は、保留中の Resume の質問や本物の会話的な応答が正常に終了することを許可します。別の Copilot セッションが、引数なしの `next` でその待ちに回答したり先へ進めたりすることはできません。明示的な `next --resume` が選択肢を再提示します。プロンプト本文とルールの内容は、調整マーカーには永続化されません。
