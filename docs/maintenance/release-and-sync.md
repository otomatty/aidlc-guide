...
- **正規名とファイル名は一致しないことがあります**（`build-test-results` → `test-results.md`）。対応はステージファイルの `outputs:` フロントマター行から解決します。厳密な語幹一致か、長さの揃った位置対応のどちらかで裏が取れた場合だけ「解決済み」とし、`.md` を推測しただけのものは**未解決として書き込みを拒否**します（`outputs:` 行が無い場合だけでなく、行はあるがその成果物を挙げていない場合も同様）。推測したファイル名は、存在しないファイルを指して I/O リンクを黙って殺すためです。
