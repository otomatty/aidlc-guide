...
- [x] `packages/vscode-extension/src/doctor-output.ts` に2.8.2を追加し、`doctor-messages-ja.ts` の新診断・対処文を翻訳。正常・警告・異常を検証。
...
- [x] readerの `effectiveness/events.ts`・`derive.ts` で要求IDを照合。別要求、Unit・attempt・Workflowの混同、欠損・不一致・古いレビューを回帰検証。
...
検証: drift **0件**、PR #96の追加修正を取り込んだ後の `VITEST_MAX_WORKERS=4 bun run check` 成功（**2,754件成功・5件skip**）。VSIXを再生成し、日本語100ページが作業ツリーと一致、更新待ち注記0件を確認。本体更新時は一時プロファイルのCursor拡張ホストで、同梱docs 2.8.2・セットアップ／更新画面を確認済み。利用中のエディターへのインストール・公開は未実施。

main `6db0b7b` 取り込み後も全体チェック成功（**2,789件成功・5件skip**）、drift **0件**。7ファイルの競合は、main側の導入処理の修正と2.8.2対応を保持して解消した。
