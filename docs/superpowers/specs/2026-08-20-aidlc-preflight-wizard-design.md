...
- サニタイズは単一関数 `buildComposeCommand(text: string): string | null`
  ...
  正規化（除去）を仕様とする。
- **8,000 文字上限（`PREFLIGHT_TEXT_MAX`）の適用**: 各層が独立に最終段で
  適用する — textarea は `maxLength` で入力自体を制限、`/api/preflight` は
  trim 後に切り詰め、`sanitizeComposeText` は除去・正規化の後に切り詰める。
  層間で適用順を厳密一致させることは要件ではない: プレビューは注記付きの
  「見通し」であり、実行内容の正はサニタイズ済みコマンド 1 点（本節冒頭の
  信頼境界）にある。サニタイズ後が空なら `buildComposeCommand` は `null`
  を返し、ホストは何もしない。ターミナル経路とクリップボード経路は同一の
  組み立て結果（単一の `command` 変数）を共有する。
