...
> 本書は当初の方式の設計記録です。現在は、既定20分のしきい値による長いログ空白の除外、承認待ち・明示的な中断の分離を実装しています。翌日への持ち越しなどの長い空白も除外しますが、記録のない短い休憩は判別できません。実装した改善方式の設計は[ステージ時間の算出改善設計](2026-09-15-stage-timing-session-design.md)、現行の表示の説明は[ステージ時間の算出方法](../../guides/stage-timing.md)を参照してください。
