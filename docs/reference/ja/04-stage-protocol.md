...
| `stage-protocol-construction.md` |Bolt ゲート、Build-and-Test 失敗ループバック、Construction の質問、Unit 単位の反復、受領記録、およびウェーブ |セッション最初の Construction ディレクティブ時、およびすべての invoke-swarm 時 |
...
1. **呼び出し。** すべてのディスパッチの前 — 初回、NOT-READY の再呼び出し、
   ...
   まずプライマリアーティファクト上の既存の `## Review` セクションを削除します。
   レビュー履歴は監査台帳にあり、残存セクション（最悪の場合、改訂前の
   `READY`）は、書き込み前に打ち切られた再レビューがまさに誤読される対象です。
   この削除ルールにより、「現在の `## Review` セクションがない」ことは、
   すべての経路で一律に「未完了のレビュー」を意味します。その後コンダクターは
   `directive.reviewer` で指定されたエージェントに委任し、ステージ定義パス、
   Q&A ファイル、生成されたアーティファクト パス、およびフロントマターの
   検証ツールを渡します — 決してビルダーの `memory.md` や計画は渡さないため、
   レビュアーは独立した判断を下します。
...
3. **評決。** `advisory` では、通常フローにおいて両方の判定が終端です。ワークフローは学習の儀式とゲートへ進み、
   ...
   `produces[]` 成果物への書き込みは受領記録を無効化し、エンジンはゲートを拒否します。
   したがって修正は反復ループの内側で行い、終端受領記録の後には決して行いません。
   判定に付随する提案は適用せず、ゲートで人間のために引用します。

反復予算はエンジンが強制します。`aidlc-log.ts review` は、`--iteration` がそのステージの
実効予算を超える `REVIEW_REQUESTED` を拒否するため、数え損ねたコンダクターが無制限に
...
レビュアーのディスパッチが失敗した、タイムアウトした、`REVIEW_REQUESTED` の後で
