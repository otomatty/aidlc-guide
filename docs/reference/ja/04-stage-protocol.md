...
- 統合された回答サマリーを提示し、続いて **Looks correct** と **Request changes** の
  選択肢を持つ構造化された確認を提示します。確認を単なる散文で求めてはいけません。
...
1. **呼び出し。** すべてのディスパッチの前 — 初回、NOT-READY の再呼び出し、
   ...
   まずレビューリクエストを記録します。ソース陳腐化からの復旧においては、
   ...
   取得してください。リクエストが成功した後、コンダクターはプライマリ
   アーティファクト上の既存の `## Review` セクションを削除し、その後
   `directive.reviewer` で指定されたエージェントに委任します。レビュー履歴は
   監査台帳にあるため、残存セクションが新しい判定と誤認されることは
   ありません。リクエストが未対応のままである間、ゲートと完了はブロックされた
   ままです。レビュアーがセクションを置き換えた後、コンダクターは判定を記録する
   前に `--retry-pending` を使って同じ保留中リクエストを現在のバイト列に
   バインドします。このフィンガープリントの再バインドは、未完了の試行の後に
   1 回だけ許可されるレビュアーの再ディスパッチとは別物であり、どちらも追加の
   レビュー反復や復旧の割り当てを消費しません。レビュアーはステージ定義パス、
   Q&A ファイル、生成されたアーティファクト パス、およびフロントマターの
   検証ツールを受け取ります — 決してビルダーの `memory.md` や計画は渡されない
   ため、レビュアーは独立した判断を下します。
2. **レビュー。** `adversarial` のレビューは敵対的レビュー契約の下で実行されます。レビュアーは
   ...
   列挙された検証ツールを実行し、プライマリアーティファクトにちょうど 1 つの
   `## Review` セクションを、ちょうど 1 行の判定行（**READY** または
   **NOT-READY**）とともに追記します。レビュアーは厳格なターン予算
...
3. **評決。** `advisory` では、通常フローにおいて両方の判定が終端です。ワークフローは学習の儀式とゲートへ進み、
   そこで指摘事項が人間の取捨選択のため逐語的に引用されます（`reviewer_max_iterations` は
   1 で、エンジンが強制します）。`adversarial` では、READY → 学習の儀式に進み、次にゲートへ。
   ...
   判定は、正しくパースできたときにのみカウントされます。すなわち、ちょうど 1 つの
   現在の `## Review` セクションに、ちょうど 1 つの正規の判定トークンがある場合です。
   セクションの欠落（上限に達した、またはクラッシュしたレビュアーは書き込まずに
