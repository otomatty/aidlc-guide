...
- **進捗なしカウンタ** — フックは有界な再帰記録を永続化します。Copilot では、セッションスコープの調整マーカーがこの記録を、監査の長さではなくワークフローとディレクティブの進捗でキー付けします。すなわち、アクティブなステージとユニット、ディレクティブの種類とパート位置、完全な継続トークンのハッシュ、ワークフロー状態のダイジェスト、所有者エポック、アクティブな Resume 状態です。`report`、ディレクティブの遷移、または実際の引き継ぎはこの署名を変えるため、健全なループはリセットされる一方で、監査だけのトラフィックが進捗を捏造することはできません。他のハーネスは既存の有界なガードファイルと進捗署名を維持します。署名が連続ブロック間で変わらなければ、カウンタは増加します。進捗なしの連続が上限に達すると、すなわち `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`（既定値は**実行モード依存**で、対話実行では 2、自律構築では 8。対話では、会話中または一時停止中の人間を 1 回の促し後に解放するため 2、自律では無人ループを完了まで進めるため 8）に達すると、フックはターンを**解放**し（停止を許可し）、したがって固着ループは必ず手を放します。明示的な `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP` は両既定値を上書きします。
...
**ガード対象外ではフェイルオープン。** 状態ファイル不在、別のステージ、別のエージェントやツール、壊れた標準入力、内部エラーのいずれでも呼び出しは許可されます。いったん code-generation の開発者ディスパッチと特定された後は、対象の証跡が欠けている、あるいは曖昧であればブロックします。決定論的な切替スイッチ `AIDLC_DISABLE_PLAN_APPROVAL_GUARD=1` は、この PreToolUse フックだけを無効化します。自律側の `aidlc-swarm.ts prepare` の前提条件は意図的に無効化**しません**。ヘッドレスのワーカーハーネスにはディスパッチフックのシームが無いことがあるため、`prepare` が、未承認のワークツリーのファンアウトを防ぐ独立したハード境界であり続けます。この境界を回避したい場合は、フックを無効化するのではなく、計画の証跡を復元・承認するか、構築をゲート付きモードに戻してください。Kiro CLI ではコンダクターエージェントが自身の `subagent` マッチャーにガードを登録し（アダプターがクルースキーマを翻訳）、Codex では `spawn_agent` の PreToolUse シームに乗り、opencode ではプラグインが `task` ディスパッチの前に参照します。Kiro IDE では他のガードと同様、散文のみの制約として文書化されています。
