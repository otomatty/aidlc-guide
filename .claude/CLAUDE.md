...
- **Model provider**: The shipped `.claude/settings.json` does not select a provider, model, region, or reasoning effort. Claude Code keeps your existing subscription, API, Bedrock, or enterprise-gateway configuration. Run `bun .claude/tools/aidlc.ts config providers` to record and validate a project provider choice.
...
- **Document skill** (user-invocable): `.claude/skills/aidlc-knowledge/`, typed as `bun .claude/tools/aidlc.ts-knowledge`; the framework CLI also exposes `bun .claude/tools/aidlc.ts knowledge <verb>`. Also standalone — outside the lifecycle graph — but classified `read-write`, unlike the read-only session skills below: it changes the document catalog and emits document audit events. It never advances the workflow stage pointer and never approves a gate. See "Document knowledge" under "Where things live" in the project's root `AGENTS.md` (Claude Code and Copilot readers find it under "Shared AI-DLC onboarding" in this file).
