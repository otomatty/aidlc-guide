...
description: >
  ...
  `bun .claude/tools/aidlc.ts engine runtime summary`. Never mutates workflow state,
...
`bun .claude/tools/aidlc.ts engine runtime summary --json` — the materialised,
...
```bash
bun .claude/tools/aidlc.ts engine runtime summary --json
```
