...
```
bun .cursor/tools/aidlc.ts engine worktree info --slug <kebab-slug>
```
...
`bun .cursor/tools/aidlc.ts engine worktree list` reports parsed Bolt directories under the
...
`bun .cursor/tools/aidlc.ts engine worktree discard --slug <slug>` sets aside the working-tree
...
```json
{
  ...
  "restore_hint": "bun .cursor/tools/aidlc-worktree.ts restore --slug onboarding-wizard --parked 20260918T123456Z --repo . --intent 260918-onboarding --space default",
...
`bun .cursor/tools/aidlc.ts engine worktree <args...>`, passing each saved `args` element exactly
...
```
bun .cursor/tools/aidlc.ts engine worktree restore --slug <slug> [--parked <stamp>] [--raw] [--repo <name|.>] [--intent <intent>] [--space <space>]
```
...
```
bun .cursor/tools/aidlc.ts engine worktree purge --slug <slug> [--parked <stamp> | --older-than <days>] [--repo <name|.>] [--intent <intent>] [--space <space>]
```
