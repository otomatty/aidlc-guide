...
```bash
bun .cursor/tools/aidlc.ts engine bolt checkpoint --action status --unit "<unit>" --kind <unit|skeleton>
```
...
```bash
bun .cursor/tools/aidlc.ts engine bolt checkpoint --action verify --unit "<unit>" --kind <unit|skeleton>
```
...
recoverable with `bun .cursor/tools/aidlc.ts engine worktree restore --slug <slug>` in a
