...
```bash
bun .cursor/tools/aidlc.ts engine testing-posture verify --unit "<unit>"
bun .cursor/tools/aidlc.ts engine testing-posture verify --unit "<unit>" --project-dir "<recorded worktree>"
```
...
Add `--resume-existing` to the ordinary `bun .cursor/tools/aidlc.ts engine swarm prepare` call
...
```bash
bun .cursor/tools/aidlc.ts engine bolt swarm-checkpoint --action status --batch <N> --units "<comma-separated emitted Units>"
```
...
```bash
bun .cursor/tools/aidlc.ts engine bolt swarm-checkpoint --action ask --batch <N> --units "<Units>" --session "<session ID>"
```
...
bun .cursor/tools/aidlc.ts engine bolt swarm-checkpoint --action approve --batch <N> --units "<Units>" --session "<session ID>" --user-input 'Approve'
# Only when human_required is false:
bun .cursor/tools/aidlc.ts engine bolt swarm-checkpoint --action approve --batch <N> --units "<Units>"
# Only after a real human Request Changes:
bun .cursor/tools/aidlc.ts engine bolt swarm-checkpoint --action reject --batch <N> --units "<Units>" --session "<session ID>" --user-input 'Request Changes' --reason '<human feedback>'
```
...
the human gate, run `bun .cursor/tools/aidlc-worktree.ts merge --slug
...
`bun .cursor/tools/aidlc.ts engine worktree <args...>`, passing each listed `args` element exactly
