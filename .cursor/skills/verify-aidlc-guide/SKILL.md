...
- `GET {origin}/` is 200 and the HTML includes `id="root"` (SPA, not API-only). Connection refusal, a hung header/body read (8s abort), or a non-HTML body fails as `{ ok: false }` JSON, not an uncaught exception. `launch` reuse uses the same deadline; `stop` does not require a healthy origin.
