...
description: >
  ...
  `bun .cursor/tools/aidlc.ts engine runtime summary`; prose comes from the artefacts. Writes
...
```bash
bun .cursor/tools/aidlc.ts engine runtime summary --json
```
...
Run `bun .cursor/tools/aidlc.ts engine runtime summary --json`.
