...
2. AI-DLC を使うツールをチェックボックスで複数選び、「インストールして設定」をクリックします。本体がない場合は公式のネイティブ版 2.8.1 を導入し、選択したツールを順に設定します。
...
あとからツールを追加する場合は、ダッシュボードの **設定 → aidlc-workflowsのインストール → インストール画面を開く** を使います。設定済みのツールはスキップし、既存プロジェクトと同じ版で追加します。結果はツールごとに表示され、一部が失敗した場合はそのツールを選んで再実行できます。
