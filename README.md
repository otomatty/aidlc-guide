...
既存プロジェクトの固定バージョンは維持します。マシンの CLI は、プロジェクトが古い版を指定していても検証済みの 2.9.0 に更新できます（確認ダイアログあり）。未導入のバージョンを自動取得できるのは 2.9.0 に限ります。マシンの既定CLIがそれより新しい場合も、更新画面から 2.9.0 を追加導入でき、処理後は新しい既定バージョンを維持します。
