...
インストール画面は、ダッシュボードの **設定 → aidlc-workflowsのインストール → インストール画面を開く** からも開けます。複数ツールの一括設定と、設定済みプロジェクトへの別ツール追加ができます。Guide は公式コマンドで生成した設定からツールごとのファイルを取り込み、既存の共通設定・成果物を保持します。ツールの追加・更新は拡張の画面から行ってください。
