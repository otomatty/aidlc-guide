...
ステージ一覧では、選択中の作業に Claude Code の使用記録がある場合、ステージごとの実モデル名を表示します。記録がなければ「デフォルト」と表示します。表示元の選択は不要です。複数のモデルが記録されていれば併記します。使用記録は再実行やレビューを含む累計で、現在のモデルや担当別の割り当てを示すものではありません。新しいモデル名も記録から表示でき、モデル一覧への登録は不要です。この表示からモデルは変更されません。
